...
const playMusic = (track, pause = false) => {
    ...
    if (!pause) {
        currentsong.play()
        play.src ="pause.svg"//gaana jbh play hora hain toh pause wala svg hona chaiye
    }
    ...
    for (let index = 0; index < array.length; index++) {
        ...
        if (e.href.includes("/songs/")) {
            ...
            cardContainer.innerHTML = cardContainer.innerHTML + `  <div data-folder="${folder}" class="card">
                        ...
                        <img width ="180" height = "180" src="/songs/${folder}/cover.jpeg" alt="">
...
async function main() {
    ...
    play.addEventListener("click", () => {
        if (currentsong.paused) {
            currentsong.play()
            play.src = "pause.svg"//done to change the play and pause buttons while the songs are played and paused
        }
        else {
            currentsong.pause()
            play.src = "play.svg"//done to change the play and pause buttons while the songs are played and paused
        }
    })
